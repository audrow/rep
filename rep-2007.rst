...
.. note::
   ...
   We may need to add additional API to match the service type support to this custom type support.

.. code-block:: cpp

   using SetBool = std_srvs::srv::SetBool;

   struct MySetBoolResponse : SetBool::Response {
     // From inheriting from SetBool::Response
     // bool success;
     // std::string message;

     // This adapter has a custom convenience method
     bool is_message_even_number_of_chars() {
       return (message.size() % 2) == 0;
     }
   };

   struct MySetBoolTypeAdapter {
     using Request = TypeAdapter<bool, SetBool::Request>;

     // We're only interested in the message part of response
     using Response = TypeAdapter<MySetBoolResponse, SetBool::Response>;
   };

   auto node = rclcpp::Node::make_shared("service_node");
   auto client = node->create_client<MySetBoolTypeAdapter>("set_bool");
   auto service = node->create_service<MySetBoolTypeAdapter>(
     "set_bool",
      [](
        const std::shared_ptr<rmw_request_id_t> request_header,
        const std::shared_ptr<bool> request,
        std::shared_ptr<MySetBoolResponse> response) -> void
      {
        (void)request_header;
   ...
   if (result) {
     RCLCPP_INFO_STREAM(node->get_logger(), 
       "The message is an even number of chars: " << 
       result->is_message_even_number_of_chars() ? "true" : "false");
   } else {
...
.. code-block:: cpp
   ...
   // TODO make it more concrete with Fibonacci
